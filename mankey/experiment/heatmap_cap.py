...
def construct_dataset(is_train: bool) -> (SupervisedKeypointDataset, SupervisedKeypointDatasetConfig):
    ...
    db_config.data_dir = '/home/rpm/Lab/cap_bottle/data_generation/datasets/train_5k' if is_train else '/home/rpm/Lab/cap_bottle/data_generation/datasets/test_100'
...
def main():
    checkpoint_dir = os.path.join(os.path.dirname(__file__), 'ckpnt-cap')
    # train(checkpoint_dir)

    shutil.rmtree('viz-cap', ignore_errors=True)
    os.makedirs('viz-cap', exist_ok=True)
    visualize(os.path.join(checkpoint_dir, 'checkpoint-116.pth'), 'viz-cap')
